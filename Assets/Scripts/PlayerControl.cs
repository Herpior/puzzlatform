...
public class PlayerControl : CharacterControl
{
	...
	public float jumpForce = 1000f;			// Amount of force added when the player jumps.
        ...
        public float groundCheckRadius = 0.4f;   // Radius of ground check circle, also used for vine check
        public LayerMask groundLayer;

        public Transform waterCheck;           // A position marking the center of circle where to check if the player is in water or near vines.
        ...
        public bool vined = false;           // Whether or not the player is near vines or other climbable things.
        public LayerMask vineLayer;
        public bool climbing = false;           // Whether or not the player is climbing vines or other climbable things.
	...
	void Update()
	{
		// The player is grounded if a linecast to the groundcheck position hits anything on the ground layer.
            ...
            vined = Physics2D.OverlapCircle(waterCheck.position, groundCheckRadius, vineLayer);
            ...
            jump = false;
            if (vert > 0 && vined && !climbing)
            {
                //climbing = true;
                //rigi.velocity = new Vector2(0.0f, rigi.velocity.y);
            }
            else if (vert > 0 && (grounded||(climbing&&hori!=0)) && now > canJumpAfter && !watered)
            {
                jump = true;
                canJumpAfter = now + jumpCoolDownInSeconds;
                climbing = false;
            }
            else if (climbing&&hori!=0&&vert<0){
                climbing = false;
            }
            else if (climbing&&!Physics2D.OverlapCircle(groundCheck.position, groundCheckRadius, vineLayer)){
                climbing = false;
            }
	}
	...
	void FixedUpdate ()
	{
        ...
        if (climbing && rigi.velocity.y < 0 && v == 0)
        {
            rigi.velocity = new Vector2(rigi.velocity.x, 0.0f);
        }
	}
